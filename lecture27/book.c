...
// HW # 7: exercise: write a function to Find a book in the collection based on its Id
// finish the function below
Book FindBook(BookCollection* collection, int id)
{
   Book b;
   b.BookId = -1;
   for(int i = 0; i < CollectionLength(collection); ++i)
   {
     if(collection->books[i].BookId == id)
     {
       return collection->books[i];
     }
   }
   return b; /*  note returns copy of b */
}

//HW # 7: exercise: write a function to Delete a book based on its Id
bool DeleteBook(BookCollection* collection, int id)
{
  ...
  for(int i = 0; i < CollectionLength(collection); ++i)
  {
     if(collection->books[i].BookId == id)
     {
       for(int j = i; j < CollectionLength(collection) - 1; ++j)
          collection->books[j] = collection->books[j+1];
       collection->len--;
       return true;
     }
  }
