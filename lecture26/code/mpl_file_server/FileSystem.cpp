...
int main(int argc, char* argv[])
{
  ...
  std::string dsktopSpec(getenv("HOME"));
  dsktopSpec += std::string("/Desktop");
  std::cout << "\n  dsktopSpec = " << dsktopSpec;

  std::cout << "\n  saving current directory";
  std::string currDir = Directory::getCurrentDirectory();
  std::cout << "\n  setting current directory to dsktopSpec";
  Directory::setCurrentDirectory(dsktopSpec);
  std::cout << "\n  currDir = " << Directory::getCurrentDirectory();
  
  std:: cout << "\n  getting absolute path for relative path = .";
  std::string testPath = Path::getFullFileSpec(".");
  std::cout << "\n  full path is: " << testPath;

  std::cout << "\n  relative path to file in desktop = ";
  std::cout << Path::getFullFileSpec("./Test1.txt");
  ...
  if(argc > 1)
    ...
    fn1 = std::string(getenv("HOME")) + "/code/FileSystem/FileSystem.h";
  ...
  {
    fn1 = std::string(getenv("HOME")) + "/code/FileSystem/FileSystem.h";
    fn2 = std::string(getenv("HOME")) + "/code/FileSystem/FileSystem.cpp";
  }
  FileInfo fi1(fn1);
  FileInfo fi2(fn2);
  ...
  std::string srcFile = "/FileSystem.o";
  std::string src = Directory::getCurrentDirectory() + srcFile;
  std::cout << "\n  src = \"" << src << "\"";
  std::string dstFile = "/copy.o";
  std::string dst = std::string(getenv("HOME")) + "/code/FileSystem/TestDst" + dstFile;
